...
var smartCheckout = {
    trans: function(key) {
        ...
        if (typeof window.smartCheckoutLang != "undefined" && typeof window.smartCheckoutLang == "object") {
            Lang = window.smartCheckoutLang;
        }
    ...
    getScriptURL: function() {
        return jq("#smart-app-script, #theapp").attr("src");
        /*
    ...
    blockUI: function(message) {
        var button = "";
        if (message) {
            var closeText = this.trans('close');
            if (!closeText) {
                closeText = 'Close';
            }
            button = '<br><br><button type="button" class="block-ui-button" onclick="jq.unblockUI();">' + closeText + '</button>';
        }
        if (!message) {
            message = this.trans('please_wait');
            if (!message) {
                message = '<img src="//assets/img/block-ui-loading.gif" alt="Loading">';
            }
        }
        var html = '<div class="block-ui-container"> \
	                    <div class="block-ui-message">' + message + button + '</div>  \
	                </div>';
        jq.blockUI({
            message: html,
            timeout: 15000,
    ...
    config: function(key) {
        var Config = {};
        if (typeof window.smartCheckoutConfig != "undefined" && typeof window.smartCheckoutConfig == "object") {
            Config = window.smartCheckoutConfig;
        }
    ...
    getUrlParameter: function(key) {
        ...
        for (i = 0; i < urlVariables.length; i++) {
            ...
            if (parameterName[0] === key && typeof parameterName[1] !== "undefined") {
    ...
    init: function() {
        smartCheckout.blockUI();
        jq(window).load(function() {
            smartCheckout.unblockUI();
        });
        var self = this,
            appUrl = this.getAppUrl(),
            sessionCode = '';
        if (jq('meta[name="csrf-token"]').length) {
            jq.ajaxSetup({
                headers: {
        ...
        var $smartCheckoutForm = jq(".smart-checkout-form, .thecheckout");
        var locale = null,
            countryCode = null;
        var countryInput = jq('[name="country"]', $smartCheckoutForm);
        if (!countryInput.length) {
            countryInput = jq('[name="country_code"]', $smartCheckoutForm);
        }
        if (countryInput && countryInput.length) {
            countryCode = countryInput.val();
        } else if ($smartCheckoutForm.data("country-code")) {
            countryCode = $smartCheckoutForm.data("country-code");
            jq('[name="country_code"]', $smartCheckoutForm).remove();
            jq('<input>').attr({
                type: 'hidden',
                name: 'country_code',
                value: countryCode
            }).appendTo($smartCheckoutForm);
        }
        var localeInput = jq('[name="locale"]', $smartCheckoutForm);
        if (localeInput && localeInput.length) {
            locale = localeInput.val();
        } else if ($smartCheckoutForm.data("locale")) {
            locale = $smartCheckoutForm.data("locale");
            jq('[name="locale"]', $smartCheckoutForm).remove();
            jq('<input>').attr({
                type: 'hidden',
                name: 'locale',
                value: locale
        ...
        if (!jq('[name="ref"]', $smartCheckoutForm).length) {
            var ref = self.getUrlParameter("ref");
            if (ref.trim() === "") {
                ref = self.getUrlParameter("utm_source");
            }
            ref = ref.trim();
            if (ref !== "") {
                jq('<input>').attr({
        ...
        jq(".smart-checkout-form, .thecheckout").submit(function(e) {
            e.preventDefault();
            var $form = jq(this);
            self.blockUI();
            jq.ajax({
                ...
                success: function(data) {
                    ...
                    self.unblockUI();
                    if (data.message) {
                        self.blockUI(data.message);
                    }
                    if (data.reset_form) {
                        jq("input[type=text]", $form).val("");
                        jq("input[type=email]", $form).val("");
                        jq("select", $form).val("");
                        jq("textarea", $form).val("");
                    }
                },
                error: function(data) {
                    var errorMesssge = new Array();
                    if (data.responseJSON) {
                        for (key in data.responseJSON) {
                            errorMesssge.push(data.responseJSON[key]);
                        }
                    }
                    errorMesssge = errorMesssge.join("<br>");
                    self.blockUI(errorMesssge);
                }
            });
        });
        var $smartCountrySelect = jq(".smart-country-select, .thecountry", $smartCheckoutForm),
            $smartCitySelect = jq(".smart-city-select, .thecity", $smartCheckoutForm),
            $smartDistrictSelect = jq(".smart-district-select, .thedistrict", $smartCheckoutForm),
            $smartPaymentSelect = jq(".smart-payment-select, .thepayment", $smartCheckoutForm);


        if ($smartPaymentSelect.length) {
            jq.ajax({
                type: "POST",
                dataType: "json",
                ...
                success: function(data) {
                    self.unblockUI();
                    if (data.html) {
                        $smartPaymentSelect.html(data.html);
                    }
                    $smartCitySelect.trigger("change");
                },
                error: function() {
                    self.unblockUI();
                }
        ...
        if ($smartCountrySelect.length) {
            jq.ajax({
                ...
                success: function(data) {
                    self.unblockUI();
                    if (data.html) {
                        $smartCountrySelect.html(data.html);
                    }
                    $smartCitySelect.trigger("change");
                },
                error: function() {
        ...
        if ($smartCitySelect.length) {
            jq.ajax({
                ...
                success: function(data) {
                    self.unblockUI();
                    if (data.html) {
                        $smartCitySelect.html(data.html);
                    }
                },
                error: function() {
                    self.unblockUI();
        ...
        $smartCountrySelect.on("change", function() {
            var $this = jq(this);
            var $cityElement = $smartCitySelect;
            if ($this.data("city-element")) {
                $cityElement = jq($this.data("city-element"), $smartCheckoutForm);
            }
            self.blockUI();
            jq.ajax({
                type: "POST",
                ...
                success: function(data) {
                    self.unblockUI();
                    if (data.html && $cityElement && $cityElement.length) {
                        $cityElement.html(data.html);
                    }
                    $smartCitySelect.trigger("change");
                },
                error: function() {
        ...
        $smartCitySelect.on("change", function() {
            var $this = jq(this);
            var $districtElement = $smartDistrictSelect;
            if ($this.data("district-element")) {
                $districtElement = jq($this.data("district-element"), $smartCheckoutForm);
            }
            self.blockUI();
            jq.ajax({
                type: "POST",
                ...
                success: function(data) {
                    self.unblockUI();
                    if (data.html && $districtElement && $districtElement.length) {
                        $districtElement.html(data.html);
                    }
                },
                error: function() {
                    self.unblockUI();
